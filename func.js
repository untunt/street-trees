...
function setColorFromFiltered() {
    paths = d3.select("#species").selectAll("path.tract")._groups[0];
    paths.forEach(d => {
        row = filtered.find(e => e.boro_ct2010 == d.__data__.properties.boro_ct2010);
        if (row === undefined) {
            color = undefinedColor;
        } else {
            color = mapColor(row.count);
        }
...
function filterData(spc="all", street="all") {
    filtered = counts.filter(e => e.spc_common == spc && e.street_suffix == street);
...
function showData() {
    ...
    container.selectAll("path").data(tracts.features)
        ...
        .attr("fill", d => {
            ...
            row = filtered.find(e => e.boro_ct2010 == d.properties.boro_ct2010);
            ...
            return colorRange(mapColor(density));
        })
    ...
    container.append("g")
        ...
        .on("mouseover", (d, i) => {
            filterData(spc=d.spc_common);
            setColorFromFiltered();
